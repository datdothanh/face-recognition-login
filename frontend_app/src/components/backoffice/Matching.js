...
import {
  detectFacesForMatching,
  getPhotoA,
  getPhotoB,
  resetMatchState,
  setMatchError,
  setMatchImage,
} from "../../features/dashboard/matchSlice";
...
export const Matching = () => {
  ...
  const handleUpload = (e) => {
    ...
    } else if (input.files && input.files[0] && input.files[0].size > 5000000) {
      dispatch(resetMatchState(source));
      dispatch(
        setMatchError({ source, error: "The IMAGE file too large (max 5 MB)." })
      );
    } else {
      dispatch(resetMatchState(source));
    ...
    <div className="col-12 my-box mb-4">
      ...
      <div className="row info mb-4">
        <div className="col-sm-12">
          <p>
            By loading two images containing one and only one face each, the
            table below shows the detail of the face as detected by the neural
            network and the Euclidean and Manhattan distances calculated
            starting from the vectorial representation of the two faces.
          </p>
          <p>
            The greater the similarity, the smaller will be the distances
            between the two faces.
          </p>
          <p>
            Subsequently, the numerical features of the detected faces are shown
            in the graph below. The greater the similarity, the more overlapping
            the two lines will be between the two faces.
          </p>
        </div>
      </div>
      <div className="row">
        <div className="col-sm-6">
          ...
          <div className="image-container">
            <img
              id="photo_A"
              alt="profile picture"
              src={photoA.src != null ? photoA.src : user.registerPic}
              onLoad={() => handleFaceDetection("photo_A")}
            />
            <div
              className="my-icon zoom-in"
              onClick={(e) => handleClickUpload("photo_A")}
            >
              <svg
          ...
          {user.loginPic || photoB.src != null ? (
            <div className="image-container">
              <img
                id="photo_B"
                alt="profile picture"
                src={photoB.src != null ? photoB.src : user.loginPic}
                onLoad={() => handleFaceDetection("photo_B")}
              />
              <div
                className="my-icon zoom-in"
                onClick={(e) => handleClickUpload("photo_B")}
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
            ...
            <table className="table table-bordered">
              ...
              <tbody>
                <tr>
                  ...
                  <td>
                    {photoA.status === "pending" ||
                    photoB.status === "pending" ? (
                      <LoaderSlim status={"pending"} />
                    ) : photoA.faces.length === 1 &&
                      photoB.faces.length === 1 ? (
                      <div
                        className={
                          "distance-score alert " +
                          (Math.round(
                            euclideanDistance(
                              photoA.faces[0].descriptor,
                              photoB.faces[0].descriptor
                            ) * 10000
                          ) /
                            10000 >
                          0.5
