...
import {
  ...
  setDetectImageError,
  setDetectTime,
} from "../../features/dashboard/detectSlice";
...
export const Detection = () => {
  ...
  const handleUpload = () => {
    ...
    if (input.files && input.files[0] && input.files[0].size < 5000000) {
      ...
      dispatch(setDetectImageError("The IMAGE file too large (max 5 MB)."));
    } else {
  ...
  const handleFaceDetection = () => {
    ...
    dispatch(detectFacesByNet(nets[0])).then((r0) => {
      if (r0.meta.requestStatus === "fulfilled" && r0.payload.length) {
        renderFaceDetected(
          document.getElementById(nets[0] + "_container"),
          r0.payload
        );
        ...
        dispatch(
          setDetectTime({
            net: nets[0],
            time: Math.round(((t0_fin - t0_init) / 1000) * 10000) / 10000,
          })
        );
      }
      ...
      dispatch(detectFacesByNet(nets[1])).then((r1) => {
        if (r1.meta.requestStatus === "fulfilled" && r1.payload.length) {
          renderFaceDetected(
            document.getElementById(nets[1] + "_container"),
            r1.payload
          );
          ...
          dispatch(
            setDetectTime({
              net: nets[1],
              time: Math.round(((t1_fin - t1_init) / 1000) * 10000) / 10000,
            })
          );
        }
        ...
        dispatch(detectFacesByNet(nets[2])).then((r2) => {
          if (r2.meta.requestStatus === "fulfilled" && r2.payload.length) {
            renderFaceDetected(
              document.getElementById(nets[2] + "_container"),
              r2.payload
            );
            ...
            dispatch(
              setDetectTime({
                net: nets[2],
                time: Math.round(((t2_fin - t2_init) / 1000) * 10000) / 10000,
              })
            );
          }
          ...
          dispatch(detectFacesByNet(nets[3])).then((r3) => {
            if (r3.meta.requestStatus === "fulfilled" && r3.payload.length) {
              renderFaceDetected(
                document.getElementById(nets[3] + "_container"),
                r3.payload
              );
              ...
              dispatch(
                setDetectTime({
                  net: nets[3],
                  time: Math.round(((t3_fin - t3_init) / 1000) * 10000) / 10000,
                })
              );
            }
          });
    ...
    <div className="col-12 my-box mb-4">
      ...
      <div className="row info">
        ...
        <div className="col-lg-8 pl-4">
          <p>
            The table below shows how different pre-trained neural networks
            operate on a given image, listing the face detections (if any) and
            the related score (=percentage by which the detection is actually
            considered a face).
          </p>
          <p>
            The table also compares the execution times of each single
                ...
                {Object.values(detections).map((d, i) => {
                  return (
                    <tr key={i}>
                      ...
                      <td>
                        <div
                          id={nets[i] + "_container"}
                          className={
                            "net-container " +
                            (d.error ? "alert alert-danger" : "")
                          }
                        >
                          {d.error ? d.error : "--"}
                        </div>
                      ...
                      <td>
                        <ExecTime net={nets[i]} detections={detections} />
                      </td>
                    </tr>
                  );
                })}
